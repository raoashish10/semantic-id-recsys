...
def test_build_input_resolves_known_items(populated_store):
    inp, n = build_input(["ITEM_A", "ITEM_C"], populated_store, num_levels=3, max_len=50, device=torch.device("cpu"))
...
def test_build_input_truncates_to_num_levels(populated_store):
    ...
    inp, n = build_input(["ITEM_A"], populated_store, num_levels=3, max_len=50, device=torch.device("cpu"))
    assert inp.shape[-1] == 3


def test_build_input_returns_none_for_unknown_items(populated_store):
    inp, n = build_input(["GHOST_ITEM"], populated_store, num_levels=3, max_len=50, device=torch.device("cpu"))
    assert inp is None
    assert n == 0


def test_build_input_skips_unknown_mixed(populated_store):
    inp, n = build_input(["ITEM_A", "GHOST", "ITEM_D"], populated_store, num_levels=3, max_len=50, device=torch.device("cpu"))
    assert n == 2  # only 2 resolve


def test_build_input_respects_max_len(populated_store):
    inp, n = build_input(["ITEM_A", "ITEM_C", "ITEM_D", "ITEM_E"], populated_store, num_levels=3, max_len=2, device=torch.device("cpu"))
...
def test_beam_recommend_excludes_session_items(populated_store):
    ...
    results = beam_recommend(model, populated_store, inp, num_levels=3, top_k=10, exclude_ids=exclude)
    returned_ids = {r.item_id for r in results}
    assert returned_ids.isdisjoint(exclude), "Session items must not appear in recommendations"
